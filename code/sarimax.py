        ...
        for col in columns_to_test:
            ...
            try:
                ...
                try:
                    ...
                    mae_value = mae(test[col], final_model.predict_in_sample(start=len(train), end=len(train) + len(test) - 1, X=exog_test))

                    test_col_altered = test[col].copy()
                    test_col_altered[test_col_altered == 0] = 0.1

                    mape_value = mape(test_col_altered, final_model.predict_in_sample(start=len(train), end=len(train) + len(test) - 1, X=exog_test))
                ...
                try:
                    ...
                    forecast, forecast_conf_intervals = final_model.predict(n_periods=steps_ahead, X=exog_test, return_conf_int=True)

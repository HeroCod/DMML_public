...
# Function to perform manual grid search for SARIMA parameters
def manual_sarima_grid_search(train_series, seasonal_period, d):
    best_aic = np.inf
    best_order = None
    ...
    try:
        ...
        for col in columns_to_test:
            ...
            try:
                ...
                # Model selection
                try:
                    print('Running manual SARIMA grid search to find the best model...')
                    seasonal_period = 24  # Data shows daily cycle
                    best_model, best_order, best_seasonal_order, best_aic = manual_sarima_grid_search(train[col], seasonal_period, d)

                    if best_model is not None:
                        final_model = best_model
                        mae_value = mae(test[col], final_model.get_forecast(steps=len(test)).predicted_mean)

                        test_col_altered = test[col].copy()
                        test_col_altered[test_col_altered == 0] = 0.1

                        mape_value = mape(test_col_altered, final_model.get_forecast(steps=len(test)).predicted_mean)
                ...
                try:
                    ...
                    forecast = final_model.get_forecast(steps=steps_ahead)

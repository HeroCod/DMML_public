...
plot_dir = os.path.join(current_dir, '../plots/sarimax/')
params_dir = os.path.join(current_dir, '../plots/sarimax/params/')
console_output_file = os.path.join(current_dir, '../plots/sarimax/console_output.json')
...
# Function to perform manual grid search for SARIMAX parameters
def manual_sarimax_grid_search(train_series, exog_train, seasonal_period, d):
    best_aic = np.inf
    best_order = None
    ...
    p_range = range(0, 3)
    d_range = [d]  # d is determined by differencing from stationarity tests
    q_range = range(0, 3)
    P_range = range(0, 2)
    D_range = range(0, 2)
    Q_range = range(0, 2)
    ...
    try:
        ...
        for col in columns_to_test:
            ...
            try:
                ...
                series = df[col]
                exog = df[['IS_HOLIDAY', 'IS_WEEKEND', 'SEASON']]
                ...
                exog_train = train[['IS_HOLIDAY', 'IS_WEEKEND', 'SEASON']]
                exog_test = test[['IS_HOLIDAY', 'IS_WEEKEND', 'SEASON']]

                # Model selection
                try:
                    print('Running manual SARIMAX grid search to find the best model...')
                    seasonal_period = 24  # Data shows daily cycle
                    best_model, best_order, best_seasonal_order, best_aic = manual_sarimax_grid_search(train[col], exog_train, seasonal_period, d)

                    if best_model is not None:
                        final_model = best_model
                        mae_value = mae(test[col], final_model.get_forecast(steps=len(test), exog=exog_test).predicted_mean)

                        test_col_altered = test[col].copy()
                        test_col_altered[test_col_altered == 0] = 0.1

                        mape_value = mape(test_col_altered, final_model.get_forecast(steps=len(test), exog=exog_test).predicted_mean)
                        ...
                        model_summary_str = str(final_model.summary())
                        console_output[dataset][col]['Model Summary'] = model_summary_str
                        print(model_summary_str)
                    else:
                        console_output[dataset][col]['Model Selection Error'] = 'No suitable model found'
                        continue
                except Exception as e:
                ...
                # Diagnostic Plots
                try:
                    diagnostics_fig = final_model.plot_diagnostics(figsize=(16, 8))
                ...
                try:
                    ...
                    exog_future = test[['IS_HOLIDAY', 'IS_WEEKEND', 'SEASON']]

                    forecast = final_model.get_forecast(steps=steps_ahead, exog=exog_future)

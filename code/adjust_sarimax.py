...
output_dir = os.path.join(current_dir, '../plots/sarimax/')
        ...
        for col in columns_to_test:
            ...
            try:
                ...
                try:
                    ...
                    mae_value = mae(test[col], fit_model.get_forecast(steps=len(test), exog=exog_future).predicted_mean)

                    test_col_altered = test[col].copy()
                    test_col_altered[test_col_altered == 0] = 0.1

                    mape_value = mape(test_col_altered, fit_model.get_forecast(steps=len(test), exog=exog_future).predicted_mean)
                ...
                try:
                    ...
                    exog_future = test[['IS_HOLIDAY', 'IS_WEEKEND', 'SEASON']]
                    forecast = fit_model.get_forecast(steps=steps_ahead, exog=exog_future)
